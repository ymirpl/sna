...
def handleTwitterHTTPError(e, t, wait_period=2):
    ...
    if e.e.code == 401:
        ...
        return 2
    elif e.e.code in (404, 403):
        print >> sys.stderr, 'Tweet not found (404) or not public (403), passing'
        return None
    else:
        raise e
...
def getUserInfo(
    ...
    # this is shit. 
    # ymir
    # if someone has above 1000 friends/followers, crawl only random 1000

    if len(screen_names) > 1000:
        shuffle(screen_names)
        screen_names = screen_names[:1000]
